...
import {updateFile} from "../data/update";
...
function Questions() {
  ...
  useEffect(async () => {
    ...
    await setFileData(questionsData);
    await setMount(true);
  }, []);
  useEffect(() => {
     setFileData(fileData => ({ ...fileData, questions_answers: data}));
}, [data])
  ...
  const getResult = async () => {
    setResult(0);
    let res = await checkArray.reduce(function (result, field, index) {
      result[counter[index]] = field;
      return result;
    }, {});
    for (var key of Object.keys(res)) {
      if (res[key]) {
        await setResult((result) => result + 1);
      }
    }
